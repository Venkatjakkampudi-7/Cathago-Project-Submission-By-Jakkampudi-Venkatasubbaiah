...
class Authentication{
    ...
    CheckSession(){
        ...
        .then(data => {
            ...
            if (data.Status === 'Logged In !' && data.Privillage === "user" && data.accountstatus === "active")
            {
                window.location.href = "Home.html";
            }
            else if (data.Status === 'Logged In !' && data.Privillage === "admin" && data.account_status === "active")
            {
    ...
    checkRegistrationformValidity()
    {
        ...
        let condition = (document.getElementById('password').value === document.getElementById('CFPassword').value);
        submitBtn.disabled = !(isValid && condition);
    }
    // End of the validation scripts
    
    // Registration Function : For creating new Account
    Registration()
    {
        // Create a FormData object to retrieve form inputs
        const formData = new FormData(document.getElementById('registrationfrm'));
        const data = {};
        ...
        // Send the data using Fetch API
        fetch('http://localhost:8080/Text_Analyser/Register', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        .then(data => {
            ...
            if(data.Privillage === "user" && data.Accountstatus === "active"){
                alert(`${data.Privillage} Login Successful !`);
                window.location.href = "Home.html";
            }
            else{
                alert(`${data.Privillage} Login Failure !`);
    ...
    // End of the Reigstration Function
    ...
    Signin()
    {
        ...
        .then(data => {
            ...
            else if(data.Privillage === "admin" && data.Accountstatus === "active"){
                window.location.href = "/Text_Analyser/Admin-App/Dashboard.html";
            }
            else
            {
...
const thereg = document.getElementById('registrationfrm');
thereg.addEventListener('input', rs.checkRegistrationformValidity);
thereg.addEventListener('change', rs.checkRegistrationformValidity);
...
document.getElementById('registrationbtn').addEventListener('click',rs.Registration);
